...
async def get_args(version_info, version, version_isolation_enabled, config):
    # 异步获取Java安装信息
    java_task = asyncio.create_task(async_find_java(config))
    ...
    # 异步获取类路径和Java信息
    cp_task = asyncio.create_task(get_cp(version_info, version, os_name, os_arch, version_directory, config))
    ...
    if minecraft_arguments:
        game_args.append(minecraft_arguments)
...
class MinecraftLauncher:
    ...
    async def launcher(self, version_info, version, version_cwd, version_isolation_enabled, config):
        args = await get_args(version_info, version, version_isolation_enabled, config)
